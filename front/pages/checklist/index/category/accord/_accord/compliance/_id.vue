                            ...
                            <tbody>
                              <tr :key="file.index" v-for="(file,index) in accord.legalDuty.compliance.legalFiles">
                                ...
                                <td class="text-center col-md-1">
                                  <a :href="baseUrl+'/public/download/legal'+file.id" class="btn btn-sm btn-info m-r-5" data-toggle="tooltip" title="" title="ดาวน์โหลด">
                                    <i class="fa fa-download"></i>
                                  </a>
                                </td>
                              </tr>
...
export default {
  ...
  methods: {
    ...
    approve: function() {
      var self = this
      http
        .post('/api/accord/accept/' + this.accord.id, this.accord, {
          headers: { Authorization: 'bearer ' + cookie(this).AT }
        })
        .then(response => {
          self.$router.push({
            path: '/checklist/category/accord/' + self.$route.params.accord
          })
        })
        .catch(e => {
          self.$router.replace('/checklist/login')
    ...
    notApprove: function() {
      var self = this
      http
        .post('/api/accord/notaccept/' + this.accord.id, this.accord, {
          headers: { Authorization: 'bearer ' + cookie(this).AT }
        })
        .then(response => {
          self.$router.push({
